...
function fixDefinitionCrossrefs() {
    ...
    defLink.forEach(function (link) {
        ...
        if (dfn.hasOwnProperty(link_txt)) {
            link.setAttribute('href','epub-spec.html#dfn-'+dfn[link_txt]);
        }
    });
}
...
function getSpecSrc() {
    ...
    try {
        ...
        xmlhttp.open('GET', 'epub-spec.html', false);
        xmlhttp.send();
        ...
        if (xmlhttp.status == 200) {
           ...
           console.error('Failed to read definition source file epub-spec.html. Returned status: ' + xmlhttp.status);
           return '';
        }
    }
