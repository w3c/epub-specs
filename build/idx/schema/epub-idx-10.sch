...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
	...
	<pattern id="index-ocurrence">
        <rule context="h:html">
            <assert test="exists(.//*[tokenize(@epub:type,'/s+')='index'])">At least one 'index'
                element must be present</assert>
        </rule>
    </pattern>

	<pattern id="index">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index']">
			<let name="semchilds" value="descendant::h:*[ancestor::h:*[@epub:type or self::h:ul][1] is current()]"/>
			<assert test="self::h:body|self::h:section|self::h:article|self::h:aside|self::h:nav"
				>The 'index' type is only allowed on 'body' or sectioning elements.</assert>
			<assert test="count($semchilds[self::h:h1|self::h:h2|self::h:h3|self::h:h4|self::h:h5|self::h:h6]) le 1">At most one heading element
				child must be present.</assert>
			<assert test="count($semchilds[tokenize(@epub:type,'\s+')='index-headnotes']) le 1">At most one
				'index-headnotes' child must be present.</assert>
			<assert
				...
				one or more 'index-group's.</assert>
		</rule>
	</pattern>

	<pattern id="index-headnotes">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-headnotes']">
			<assert test="self::h:header|self::h:section|self::h:article|self::h:aside|self::h:nav"
				>The 'index-headnotes' type is only allowed on 'header' or sectioning
				elements.</assert>
			<assert test="ancestor::h:*[@epub:type][1][tokenize(@epub:type,'\s+')='index']">The 'index-headnotes'
				type is only allowed as a child of 'index'.</assert>
		</rule>
	</pattern>
	...
	<pattern id="index-group">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-group']">
			<assert test="self::h:section|self::h:article|self::h:aside|self::h:nav">The
				'index-group' type is only allowed on sectioning elements.</assert>
			<assert test="ancestor::h:*[@epub:type][1][tokenize(@epub:type,'\s+')='index']">The 'index-group' type is
				only allowed on children of 'index'.</assert>
			<assert test="count(descendant::h:*[ancestor::h:*[@epub:type][1] is current()][self::h:h1|self::h:h2|self::h:h3|self::h:h4|self::h:h5|self::h:h6]) le 1">'index-group' must not have
				more than one heading child.</assert>
			<assert
				test="count(descendant::h:*[ancestor::h:*[@epub:type][1] is current()][tokenize(@epub:type,'\s+')='index-entry-list']
				           |descendant::h:ul[ancestor::h:*[@epub:type][1] is current()][not(@epub-type)])
				           =1"
                      ...
                      |h:li[parent::h:ul
                             [tokenize(@epub:type,'\s+')='index-entry-list' 
                              or (not(@epub:type)
                                  and ancestor::h:*[tokenize(@epub:type,'\s+')='index'] 
                                  and empty(ancestor::h:*[tokenize(@epub:type,'\s+')='index-headnotes']))]]">
			<let name="semchilds" value="descendant::h:*[ancestor::h:*[@epub:type or self::h:li][1] is current()]"/>
			<assert test="self::h:li">The 'index-entry' type is only allowed on 'li'
				elements.</assert>
			<assert
				...
				[tokenize(@epub:type,'\s+')='index-entry'
				 or self::h:li and parent::h:ul[
                      ...
                      or (not(@epub:type)
                          and ancestor::h:*[tokenize(@epub:type,'\s+')='index'] 
                          and empty(ancestor::h:*[tokenize(@epub:type,'\s+')='index-headnotes']))]]"
				>The '<xsl:value-of select="'$type'"/>' type is only allowed on children of elements
				with the (possibly implied) type 'index-entry'.</assert>
		</rule>
	</pattern>
	...
	<pattern id="index-locator">
		<rule
			...
			<assert test="self::h:a">The 'index-locator' type is only allowed on 'a'
				elements.</assert>
			<assert
	...
	<pattern id="index-locator-range">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-locator-range']">
			<assert
				...
				'index-locator-list'.</assert>
			<assert test="exists(descendant::h:a) and count(descendant::h:a) le 2">Must contain one
				or two 'index-locator' child.</assert>
		</rule>
	</pattern>

	<pattern id="index-xref">
		<rule
			context="h:*[tokenize(@epub:type,'\s+')=('index-xref-preferred','index-xref-related')]">
			<assert
				...
				the type 'index-term' or 'index-term-category'.</assert>
		</rule>
	</pattern>
	...
	<pattern id="index-term-category">
		<rule context="h:*[tokenize(@epub:type,'\s+')='index-term-category']">
			...
			<assert
				...
				elements) with the type 'index-xref-preferred' or 'index-xref-related'.</assert>
		</rule>
	</pattern>
