...
<xsl:template match="productionset">
  <table>
    <xsl:attribute name="style">
      <xsl:text>width: 100%; border-collpase: collapse; padding: 5px;</xsl:text>
      <xsl:if test="$ebnf.table.bgcolor != ''">
        <xsl:text>background-color: </xsl:text><xsl:value-of select="$ebnf.table.bgcolor"/><xsl:text>;</xsl:text>
      </xsl:if>
    ...
    <xsl:if test="title|info/title">
      <tr>
        <th style="text-align: {$direction.align.start}; vertical-align: top;">
...
<xsl:template match="production">
  ...
  <tr>
    <td style="text-align: {$direction.align.start}; vertical-align: top; width: 3%;">
      <xsl:text>[</xsl:text>
      <xsl:apply-templates select="." mode="label.markup"/>
      <xsl:text>]</xsl:text>
    </td>
    <td style="text-align: {$direction.align.end}; vertical-align:top; width: 10%">
      <xsl:choose>
        <xsl:when test="$recap">
          <a>
    ...
    <td style="text-align: {$direction.align.start}; vertical-align: top; width: 30%;">
      <xsl:choose>
